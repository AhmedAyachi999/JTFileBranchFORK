...
def read_table_of_contents(path: str):
    ...
    with open(path, mode='rb') as jt:
        ...
        if VERSION == JtVersion.V10d5:
            jt_toc_offset = struct.unpack("<Q", jt.read(8))[0]
        else:
...
def main():
    ...
    if VERSION != JtVersion.V10d5:
        ...
        logger.info(f"finished read of lsg segment with {len(lsg.nodes)} nodes")
    ...
    if VERSION == JtVersion.V10d5:
        logging.getLogger("shape.shape").setLevel(logging.ERROR)
        available_shape_type_ids = {entry.type.id for entry in shape_entries}
        preferred_v10_type_order = [
            ShapeLod1.SEGMENT_TYPE_ID,
            ShapeLod2.SEGMENT_TYPE_ID,
            ShapeLod0.SEGMENT_TYPE_ID,
            Shape.SEGMENT_TYPE_ID,
        ]
        selected_shape_entries = shape_entries
        for preferred_type_id in preferred_v10_type_order:
            if preferred_type_id in available_shape_type_ids:
                selected_shape_entries = [
                    entry for entry in shape_entries if entry.type.id == preferred_type_id
                ]
                logger.info(
                    f"Using V10.5 shape segment type {preferred_type_id} for point-cloud extraction "
                    f"({len(selected_shape_entries)} segments selected from {len(shape_entries)})"
                )
                break
        ...
        for entry in selected_shape_entries:
            ...
            try:
                ...
                logger.error(f"Failed to read V10.5 shape segment at {entry.offset}: {exc}")
                continue
        ...
        if not aggregated_points:
            logger.warning("No V10.5 point data could be extracted.")
            return

        point_cloud_points = np.concatenate(aggregated_points, axis=0)
        logger.info(
            f"Collected {point_cloud_points.shape[0]} V10.5 points from "
            f"{extracted_count} shapes across {len(selected_shape_entries)} selected shape segments"
        )
        ...
        if not args.no_view:
            ...
            vis.create_window(
                window_name=f"JT V10.5 Point Cloud ({point_cloud_points.shape[0]} points)"
            )
